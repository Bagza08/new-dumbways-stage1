...
var userloginSessionkos = userloginSession{}
...
func project(c echo.Context) error {
	...
	if sess.Values["Islogin"] != true {
		userloginSessionkos.Islogin = false
	} else {
		userloginSessionkos.Islogin = true
		userloginSessionkos.Name = sess.Values["name"].(string)
	}

	alldataproject := map[string]interface{}{
		"project":     result_projects,
		"datasession": userloginSessionkos,
	}
...
func formregister(c echo.Context) error {
	...
	// fmt.Println("pesan :", sess.Values["pesan"])
...
func register(c echo.Context) error {
	...
	regisName := c.FormValue("name")
	regisEmail := c.FormValue("email")
	regisPass := c.FormValue("password")

	hashedPass, errencryptedPass := bcrypt.GenerateFromPassword([]byte(regisPass), 10)

	if errencryptedPass != nil {
		return c.JSON(http.StatusInternalServerError, errencryptedPass.Error())
	}

	fmt.Println(regisName, regisEmail, regisPass)

	queryregis, errregis := connection.Conn.Exec(context.Background(), "INSERT INTO tb_user (name,email,psw) VALUES ($1,$2,$3)", regisName, regisEmail, hashedPass)

	if errregis != nil {
		return redirectpesan(c, "Registrasi GAGAL !", false, "/form-register")
	}

	fmt.Println("affect row:", queryregis)
	...
	return redirectpesan(c, "Registrasi Berhasil !", true, "/form-login")
}
...
func login(c echo.Context) error {
	...
	// check email dulu
	errlogin := connection.Conn.QueryRow(context.Background(), "SELECT id,name,email,psw FROM tb_user WHERE email=$1 ", regisEmail).Scan(&user.Id, &user.UsNama, &user.UsEmail, &user.UsHashedPass)

	if errlogin != nil {
		return redirectpesan(c, "Login GAGAL !", false, "/form-login")
	}

	errcompare := bcrypt.CompareHashAndPassword([]byte(user.UsHashedPass), []byte(regisPass))
	...
	//sett session (sukses login)

	//return redirectpesan(c, "Login Berhasil !", true, "/")

	sess, _ := session.Get("session", c)

	sess.Options.MaxAge = 10800 // 3 jam
	sess.Values["pesan"] = "Login Sukses!"
	sess.Values["status"] = true
	sess.Values["name"] = user.UsNama
	sess.Values["email"] = user.UsEmail
...
func logout(c echo.Context) error {
	...
	return redirectpesan(c, "Logout Berhasil", true, "/form-login")
}

func redirectpesan(c echo.Context, pesan string, status bool, redirectPath string) error {
	sess, errsess := session.Get("session", c)
