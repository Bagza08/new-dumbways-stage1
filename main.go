...
func project(c echo.Context) error {
	...
	query_projects, err_query_projects := connection.Conn.Query(context.Background(), "SELECT name,start_date,end_date,description,technologies,image FROM tb_project")
	...
	for query_projects.Next() {
		...
		err := query_projects.Scan(&each.StrNama, &each.StrSdate, &each.StrEdate, &each.StrDesc, &each.Fix, &each.StrImg)
		if err != nil {
			return c.JSON(http.StatusInternalServerError, err.Error())
		}
